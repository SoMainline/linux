...
static void wcd937x_io_init(struct regmap *regmap)
{
	...
	regmap_update_bits(regmap, WCD937X_SLEEP_CTL, BIT(6), BIT(6));
	usleep_range(1000, 1010);
...
{pr_err("%s\n", __func__);
	...
	micb_val = snd_soc_component_read_field(component, micb_reg, WCD937X_ANA_MICB_EN);
	cur_vout_ctl = snd_soc_component_read_field(component, micb_reg, WCD937X_ANA_MICB_VOUT);
	...
	if (micb_val == WCD937X_MICB_ENABLE)
		snd_soc_component_update_bits(component, micb_reg, WCD937X_ANA_MICB_EN, BIT(7));

	snd_soc_component_update_bits(component, micb_reg, WCD937X_ANA_MICB_VOUT, req_vout_ctl);

	if (micb_val == WCD937X_MICB_ENABLE) {
		snd_soc_component_update_bits(component, micb_reg, WCD937X_ANA_MICB_EN, BIT(6));
...
static const struct soc_enum adc2_enum =
	SOC_ENUM_SINGLE(WCD937X_TX_NEW_TX_CH2_SEL, 7, ARRAY_SIZE(adc2_mux_text), adc2_mux_text);


static const struct soc_enum rdac3_enum =
	SOC_ENUM_SINGLE(WCD937X_DIGITAL_CDC_EAR_PATH_CTL, 0, ARRAY_SIZE(rdac3_mux_text), rdac3_mux_text);
...
static int wcd937x_soc_codec_probe(struct snd_soc_component *component)
{
	...
	wcd937x->chipid = (snd_soc_component_read(component, WCD937X_DIGITAL_EFUSE_REG_0) & 0x1e) >> 1;
...
static void wcd937x_dt_parse_micbias_info(struct device *dev, struct wcd937x_priv *wcd)
{
	...
	ret = of_property_read_u32(np, "qcom,micbias4-microvolt", &prop_val);
	if (!ret)
		wcd->micb4_mv = prop_val/1000;
	else
		dev_warn(dev, "Micbias4 DT property not found\n");
}
...
static int wcd937x_bind(struct device *dev)
{
	...
	wcd937x->rxdev = wcd93xx_sdw_device_get(wcd937x->rxnode);
	if (!wcd937x->rxdev) {
	...
	wcd937x->txdev = wcd93xx_sdw_device_get(wcd937x->txnode);
	if (!wcd937x->txdev) {

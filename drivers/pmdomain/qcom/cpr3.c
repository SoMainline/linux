...
static int cpr_fuse_corner_init(struct cpr_thread *thread)
{
	...
	for (i = 0; fuse <= end; fuse++, cpr_fuse++, i++, fdata++) {
		int factor = desc->ro_scaling_factor[i][fuse->ring_osc_idx];
...
static int cpr3_corner_init(struct cpr_thread *thread)
{
	...
	for (i = 0; corner <= end; corner++, i++) {
		...
		if (prev_fuse) {
			...
			ro_fac = tdesc->ro_scaling_factor[fnum][fuse->ring_osc_idx];
...
static int cpr3_init_parameters(struct cpr_drv *drv)
{
	...
	if (IS_ERR(clk)) {
		dev_err(drv->dev, "Couldn't get the reference clock: %d\n", PTR_ERR(clk));
		return PTR_ERR(clk);
	}
...
static unsigned int cpr_get_performance_state(struct generic_pm_domain *pmdomain,
					      struct dev_pm_opp *opp)
{
...
static int cpr3_debug_info_show(struct seq_file *s, void *unused)
{
	...
	seq_printf(s, "busy = %u, ", FIELD_GET(CPR3_RESULT0_BUSY_MASK, reg));
	seq_printf(s, "step_dn = %u, ", FIELD_GET(CPR3_RESULT0_STEP_DN_MASK, reg));
	seq_printf(s, "step_up = %u, ", FIELD_GET(CPR3_RESULT0_STEP_UP_MASK, reg));
	seq_printf(s, "error_steps = %u, ", FIELD_GET(CPR3_RESULT0_ERROR_STEPS_MASK, reg));
	seq_printf(s, "error = %u, ", FIELD_GET(CPR3_RESULT0_ERROR_MASK, reg));
	seq_printf(s, "negative = %u", FIELD_GET(CPR3_RESULT0_NEG_MASK, reg));
	seq_puts(s, "]\n");

	reg = cpr_read(thread, CPR3_REG_RESULT1(tid));
	seq_printf(s, "cpr_result_1 = 0x%x\n  [", reg);
	seq_printf(s, "quot_min = %u, ", FIELD_GET(CPR3_RESULT1_QUOT_MIN_MASK, reg));
	seq_printf(s, "quot_max = %u, ", FIELD_GET(CPR3_RESULT1_QUOT_MAX_MASK, reg));
	seq_printf(s, "ro_min = %u, ", FIELD_GET(CPR3_RESULT1_RO_MIN_MASK, reg));
	seq_printf(s, "ro_max = %u", FIELD_GET(CPR3_RESULT1_RO_MAX_MASK, reg));
	seq_puts(s, "]\n");

	reg = cpr_read(thread, CPR3_REG_RESULT2(tid));
	seq_printf(s, "cpr_result_2 = 0x%x\n  [", reg);
	seq_printf(s, "qout_step_min = %u, ", FIELD_GET(CPR3_RESULT2_STEP_QUOT_MIN_MASK, reg));
	seq_printf(s, "qout_step_max = %u, ", FIELD_GET(CPR3_RESULT2_STEP_QUOT_MAX_MASK, reg));
	seq_printf(s, "sensor_min = %u, ", FIELD_GET(CPR3_RESULT2_SENSOR_MIN_MASK, reg));
	seq_printf(s, "sensor_max = %u", FIELD_GET(CPR3_RESULT2_SENSOR_MAX_MASK, reg));
	seq_puts(s, "]\n");
...
fail:
	/* Unregister all previously registered pmdomains */

...
void nt36xxx_enable_bl_crc(struct nt36xxx_data *ts)
{
	...
	nt36xxx_set_page(ts, ts->mmap->bld_crc_en_addr);
...
void nt36xxx_enable_fw_crc(struct nt36xxx_data *ts)
{
	...
	nt36xxx_set_page(ts, ts->mmap->event_buf);
...
{
	nt36xxx_write_addr(ts, ts->mmap->boot_rdy_addr, 1);

	mdelay(5);

	if (!ts->hw_crc) {
		nt36xxx_write_addr(ts, ts->mmap->boot_rdy_addr, 0);
		nt36xxx_write_addr(ts, ts->mmap->por_cd_addr, 0xa0);
	}
}
...
int nt36xxx_check_spi_dma_tx_info(struct nt36xxx_data *ts)
{
	...
	for (i = 0; i < SPI_DMA_TX_INFO_MAX_RETRIES; i++) {
		nt36xxx_set_page(ts, ts->mmap->spi_dma_tx_info);
...
{
	nt36xxx_write_addr(ts, ENG_RST_ADDR, 0x5a);
	mdelay(1);
}
EXPORT_SYMBOL_GPL(nt36xxx_eng_reset);

/* Reset the MCU into FW download mode */
void nt36xxx_bootloader_reset(struct nt36xxx_data *ts)
{
	nt36xxx_write_addr(ts, ts->swrst_n8_addr, 0x69);
	mdelay(5);

	/* Disable SPI_RD_FAST if present */
	if (ts->spi_rd_fast_addr)
		nt36xxx_write_addr(ts, ts->spi_rd_fast_addr, 0);
}
EXPORT_SYMBOL_GPL(nt36xxx_bootloader_reset);
...
int nt36xxx_check_fw_reset_state(struct nt36xxx_data *ts, u8 desired_state)
{
	...
	nt36xxx_set_page(ts, ts->mmap->event_buf | EVENT_MAP_RESET_COMPLETE);
...
int nt36xxx_get_fw_info(struct nt36xxx_data *ts)
{
	...
	ret = nt36xxx_set_page(ts, ts->mmap->event_buf | EVENT_MAP_FWINFO);
	if (ret)
		return ret;

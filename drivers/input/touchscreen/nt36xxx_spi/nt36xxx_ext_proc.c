...
static struct proc_dir_entry *NVT_touch_proc_dir;
...
int32_t nvt_extra_proc_init(void)
{
	...
	if (NVT_touch_proc_dir == NULL) {
		NVT_ERR(" %s: NVT_touch_proc_dir file create failed!\n", __func__);
		return -ENOMEM;
	}

	NVT_proc_gesture_entry = proc_create(GESTURE_CONTROL, 0666, NVT_touch_proc_dir, &nvt_gesture_fops);
	if (NVT_proc_gesture_entry == NULL) {
		NVT_ERR("create proc/gesture_control Failed!\n");
		return -ENOMEM;
	} else {
		NVT_LOG("create proc/gesture_control Succeeded!\n");
	}
...
void nvt_extra_proc_deinit(void)
{
	...
	if (NVT_proc_gesture_entry != NULL) {
		remove_proc_entry("gesture_control", NVT_touch_proc_dir);

...
/* 0x16..0x17,  0x19..0x3c - some 1-bit r/w regs*/
...
#define REG_53				0x53
 #define REG_53_FNKEY_TRISTATE		GENMASK(1, 0)
  ...
  /* 3 - unused */
 #define REG_53_FNKEY_TRISTATE		GENMASK(3, 2)
  /* 0 - res / ignore */
...
static int x13s_ec_probe(struct i2c_client *client)
{
	...
	x13s_ec_read(ec, 0x53, buf);
	pr_err("EC [0x51] = 0x%x\n", buf[0]);
	x13s_ec_write(ec, 0xcf, 0xff);
	x13s_ec_read(ec, 0xcf, buf);
	pr_err("EC [0xcf] = 0x%x\n", buf[0]);
